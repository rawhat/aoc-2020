...
pub fn part_one() -> Int {
  ...
  |> fn(ship: Ship) { abs(ship.position.x) + abs(ship.position.y) }
}

pub type ShipWithWaypoint {
  ShipWithWaypoint(ship: Position, waypoint: Position)
}

pub fn new_ship_with_waypoint() -> ShipWithWaypoint {
  ShipWithWaypoint(ship: Position(x: 0, y: 0), waypoint: Position(x: 10, y: 1))
}
...
pub fn travel_with_waypoint(
  ...
  case instruction {
    ...
    Instruction(direction: Forward, value: value) -> {
      ...
      let new_ship =
        move(with_waypoint.ship, offset.x * value, offset.y * value)
      ShipWithWaypoint(ship: new_ship, waypoint: move_pos(new_ship, offset))
    }
  }
}
